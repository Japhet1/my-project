            ...
            <div className="lg:grid grid-cols-12 gap-2 dark:text-white">
                {educationData.physical.map((education, index) => (
                    <div key={index} 
                        ...
                        `}>
                        <div>
                            <h1 className="text-xl md:text-2xl lg:text-3xl font-bold mb-4">{education.name}</h1>
                        </div>
                        <div className="leading-8 text-lg md:text-lg lg:text-xl font-semibold text-blue-950 dark:text-slate-300">
                ...
                <div  className="col-span-8 bg-gray-200 dark:bg-gray-600 text-left p-6 md:p-10 lg:p-10 rounded-md">
                    <h1 className="text-xl md:text-2xl lg:text-3xl font-bold mb-4">{educationData.online.name}</h1>
                    {educationData.online.content.map((education, index) => (
                        <div key={index} className="">
                            <div className=""><h1 className="text-lg md:text-xl lg:text-2xl font-bold">{education.name}</h1></div>
                            <div className="flex flex-wrap items-center gap-4 mb-5">
                                {education.items.map((education, id) => <p key={id} className="flex items-center text-md md:text-lg lg:text-xl gap-2 font-semibold text-blue-950 dark:text-slate-300"><BsDot/>{education.label}</p>)}  
                             </div>
                        </div>
                    ))}
                </div>
